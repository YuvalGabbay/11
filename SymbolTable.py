...
class SymbolTable:
    ...
    def __init__(self) -> None:
        ...
        self.counters = {'field:':0, 'static':0, 'local':0,'argument':0}
    ...
    def define(self, name: str, type: str, kind: str) -> None:
        ...
        if kind == 'field' or kind == 'static':
            self.class_table[name] = [type, kind, self.counters[kind]]

        else:
            self.subroutine_table[name] = [type, kind, self.counters[kind]]
    ...
    def var_count(self, kind: str) -> int:
        ...
        # Your code goes here!
        if kind == "ARG":
            return self.counters['argument']
        if kind == "VAR":
            return self.counters['local']
        else:
            return self.counters[kind.lower()]
